                                                        ...
                                                        <select class="form-select anggota-select" name="petugas[{{ $loop->index }}][id_anggota]" data-posisi="{{ $jadwal->posisi }}">
                                                            ...
                                                            @foreach($anggota as $a)
                                                                @php
                                                                    ...
                                                                    // FIXED: Check if can serve this position - berdasarkan spesialisasi saja
                                                                    $canServePosition = $a->spesialisasi->contains('posisi', $jadwal->posisi);
                                                                    
                                                                    // FIXED: Check if regular for this position - dari spesialisasi, bukan general
                                                                    $specialization = $a->spesialisasi->where('posisi', $jadwal->posisi)->first();
                                                                    $isReguler = $specialization ? $specialization->is_reguler : false;
                                                                    $prioritas = $specialization ? $specialization->prioritas : 0;
                                                                @endphp
                                                                ...
                                                                >
                                                                    ...
                                                                    @if($prioritas > 0 && !$isReguler) (Priority: {{ $prioritas }}) @endif
                                                        ...
                                                        <select class="form-select anggota-select" name="petugas[{{ $loop->index }}][id_anggota]" data-posisi="{{ $posisi }}">
                                                            ...
                                                            @foreach($anggota as $a)
                                                                @php
                                                                    ...
                                                                    // FIXED: Check specialization properly
                                                                    $canServePosition = $a->spesialisasi->contains('posisi', $posisi);
                                                                    $specialization = $a->spesialisasi->where('posisi', $posisi)->first();
                                                                    $isReguler = $specialization ? $specialization->is_reguler : false;
                                                                    $prioritas = $specialization ? $specialization->prioritas : 0;
                                                                @endphp
...
<script>
    $(document).ready(function() {
        ...
        // Function to format anggota selection with availability and skill info
        function formatAnggotaSelection(anggota) {
            ...
            const canServe = $element.data('can-serve') === 'true';
            const prioritas = $element.data('prioritas') || 0;
            ...
            // FIXED: Add visual indicators for skill level and availability
            if (canServe) {
                if (isReguler) {
                    $result.append(' <i class="fas fa-star text-warning" title="Regular Player"></i>');
                } else if (prioritas >= 8) {
                    $result.append(' <i class="fas fa-certificate text-success" title="High Priority: ' + prioritas + '/10"></i>');
                } else if (prioritas >= 5) {
                    $result.append(' <i class="fas fa-medal text-info" title="Medium Priority: ' + prioritas + '/10"></i>');
                } else {
                    $result.append(' <i class="fas fa-user text-secondary" title="Basic Priority: ' + prioritas + '/10"></i>');
                }
                
                if (!isAvailable) {
                    $result.append(' <i class="fas fa-exclamation-triangle text-warning" title="Not Available"></i>');
                }
            } else {
                $result.append(' <i class="fas fa-times-circle text-danger" title="Cannot serve this position"></i>');
            }
        ...
        function addPositionFromMaster(position) {
                    ...
                    <div class="col-md-7">
                        <div class="form-group">
                            ...
                            <select class="form-select anggota-select-new" name="petugas[${positionCounter}][id_anggota]" data-posisi="${position}" required>
                                ...
                                @foreach($anggota as $a)
                                    @php
                                        // FIXED: Check based on actual specialization
                                        $canServePosition = $a->spesialisasi->contains('posisi', '__POSITION__');
                                        $specialization = $a->spesialisasi->where('posisi', '__POSITION__')->first();
                                        $isReguler = $specialization ? $specialization->is_reguler : false;
                                        $prioritas = $specialization ? $specialization->prioritas : 0;
                                    @endphp
                                    <option value="{{ $a->id_anggota }}" 
                                            data-can-serve="__CAN_SERVE__" 
                                            data-reguler="__IS_REGULER__"
                                            data-prioritas="__PRIORITAS__">
                                        {{ $a->nama }}
                                        __INDICATORS__
                                    </option>
                                @endforeach
            ...
            // Replace placeholders with actual data for this position
            let processedRow = newRow;
            @foreach($anggota as $a)
                processedRow = processedRow.replace('__POSITION__', position);
                
                @php
                    // This will be processed client-side, but we need the template
                @endphp
            @endforeach
            
            $('#petugas-container').append(processedRow);
        ...
        // FIXED: Validation - allow high priority members even if not regular
        $('form').on('submit', function(e) {
            ...
            // FIXED: Check for members serving positions they can't handle - but allow with warning
            let hasUnskilledAssignments = false;
            $('.petugas-row select[name*="[id_anggota]"]').each(function() {
                const selectedOption = $(this).find('option:selected');
                const canServe = selectedOption.data('can-serve');
                const anggotaName = selectedOption.text().split(' (')[0]; // Remove additional info
                const posisi = $(this).closest('.petugas-row').find('input[name*="[posisi]"]').val();
                
                if ($(this).val() && canServe === 'false') {
                    hasUnskilledAssignments = true;
                    errorMessages.push(`PERINGATAN: ${anggotaName} tidak memiliki spesialisasi untuk posisi "${posisi}".`);
                }
            });
            ...
            // If there are unskilled assignments, ask for confirmation
            if (hasUnskilledAssignments) {
                if (!confirm('Terdapat penugasan anggota yang tidak memiliki spesialisasi untuk posisi tersebut. Yakin ingin melanjutkan?\n\n' + errorMessages.join('\n'))) {
                    e.preventDefault();
                    return false;
                }
            }
        ...
        // FIXED: Visual feedback based on skill and availability
        $('.anggota-select').on('change', function() {
            ...
            const prioritas = selectedOption.data('prioritas') || 0;
            
            $(this).removeClass('border-success border-warning border-danger border-info');
            
            if ($(this).val()) {
                if (canServe === 'false') {
                    $(this).addClass('border-danger'); // Red for no skill
                } else if (isAvailable === 'false') {
                    $(this).addClass('border-warning'); // Yellow for not available
                } else if (isReguler === 'true') {
                    $(this).addClass('border-success'); // Green for regular
                } else if (prioritas >= 7) {
                    $(this).addClass('border-info'); // Blue for high priority
                }
            }
        });
        ...
        $('body').on('change', 'select[name*="[id_anggota]"]', function() {
            ...
            const canServe = selectedOption.data('can-serve') === 'true';
            ...
            if (!canServe && $this.val()) {
                const posisi = $this.closest('.petugas-row').find('input[name*="[posisi]"]').val();
                if (confirm(`Anggota yang dipilih tidak memiliki spesialisasi untuk posisi "${posisi}". Yakin ingin melanjutkan?`)) {
                    // Continue
                } else {
                    $this.val('').trigger('change');
                }
