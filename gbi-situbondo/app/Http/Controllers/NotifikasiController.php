...
class NotifikasiController extends Controller
{
    ...
    public function sendPelayananReminders(Request $request)
    {
        ...
        try {
            ...
            SendDailyReminders::dispatch($when)->onQueue('notifications');
            
            Log::info("Pelayanan reminders dispatched", [
                'when' => $when,
                'user' => Auth::user()->email
            ]);
            
            return redirect()->route('notifikasi.index')
                ->with('success', 'Pengingat pelayanan sedang diproses dan akan dikirim dalam beberapa saat.');
        } catch (\Exception $e) {
            ...
            return redirect()->route('notifikasi.index')
                ->with('error', 'Terjadi kesalahan saat memproses pengingat pelayanan.');
        }
    }
    ...
    public function sendKomselReminders(Request $request)
    {
        ...
        try {
            // Use the same job but filter will be applied in the job
            SendDailyReminders::dispatch($when)->onQueue('notifications');
            
            return redirect()->route('notifikasi.index')
                ->with('success', 'Pengingat komsel sedang diproses dan akan dikirim dalam beberapa saat.');
        } catch (\Exception $e) {
            return redirect()->route('notifikasi.index')
                ->with('error', 'Terjadi kesalahan saat memproses pengingat komsel.');
        }
    }
    ...
    public function sendIbadahReminders(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('notifikasi.index')
                ->with('success', 'Pengingat ibadah sedang diproses dan akan dikirim dalam beberapa saat.');
        } catch (\Exception $e) {
            return redirect()->route('notifikasi.index')
                ->with('error', 'Terjadi kesalahan saat memproses pengingat ibadah.');
        }
    }
